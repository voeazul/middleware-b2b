...
| ----------------- | -------------- | ---------------------------------------------- | ----------------------------------------------|
| 400 Bad Request | AgentOrder.NotAuthorized | Agent is not authorized to commit order with invalid price status without requesting to override restrictions. | O agente não está autorizado a confirmar reservas com status de preço inválido sem solicitar a substituição das restrições. |
| 400 Bad Request | AuthorizationFailed.Credentials | An error occurred while authenticating. | Ocorreu um erro durante a autenticação. |
...
| 400 Bad Request | Integration.Error | The informed 'RecordLocator' is invalid! | O 'RecordLocator' informado é inválido! |
| 400 Bad Request | InvalidField.Account | The 'Account' is invalid. | A 'Conta' é inválida. |
| 400 Bad Request | InvalidField.Account.Length | The 'Account' has a maximum length of 16 characters. | A 'Conta' tem um comprimento máximo de 16 caracteres. |
...
| 400 Bad Request | InvalidField.AssistanceCode | The assistance '{0}' is invalid for the Journey '{1}-{2} - {3}'. | A assistência '{0}' é inválida para o Voo '{1}-{2} - {3}'. |
...
| 400 Bad Request | InvalidField.Code | The 'Code' must have a maximum of 4 characters. | O 'Código' deve ter no máximo 4 caracteres. |
...
| 400 Bad Request | InvalidField.ContactName.First | The 'Contact.Name.First' has a maximum length 32 characters. | O 'Contact.Name.First' tem comprimento máximo de 32 caracteres. |
...
| 400 Bad Request | InvalidField.CountryCode | An error occurred while requesting order. | Ocorreu um erro ao solicitar a reserva. |
...
| 400 Bad Request | InvalidField.customerProgram.number | Letters are not allowed. | Cartas não são permitidas. |
...
| 400 Bad Request | InvalidField.DateOfBirth | Passengers under 18 years old are not allowed with infant. | Passageiros menores de 18 anos não são permitidos com bebês. |
...
| 400 Bad Request | InvalidField.FareKey | The 'FareKey' is invalid. | O 'FareKey' é inválido. |
...
| 400 Bad Request | InvalidField.FlightNumber | Some segments for the Journey '{0}-{1} - {2}' do not support the assistance '{3}'. Please contact the customer service to complete the special assistance process. | Alguns trechos do voo '{0}-{1} - {2}' não suportam a assistência '{3}'.  |
...
| 400 Bad Request | InvalidField.Journeys.Count | The maximum number of Journeys allowed in a order is {0}. | O número máximo de voos permitidas em uma reserva é {0}. |
...
| 400 Bad Request | InvalidField.OrganizationCode | The 'OrganizationCode' is invalid. | O 'OrganizationCode' é inválido. |
...
| 400 Bad Request | InvalidField.PassengerType.Length | The 'Passengers.Type' has a maximum length of 3 characters. | O 'Passengers.Type' tem comprimento máximo de 3 caracteres. |
...
| 400 Bad Request | InvalidField.TravelDocument.IssuingCountry | The 'TravelDocuments.IssuingCountry' has a maximum length 3 characters. | O 'TravelDocuments.IssuingCountry' tem no máximo 3 caracteres. |
...
| 400 Bad Request | Journeys.Overlap | There are journeys overlapping. | Há viagens sobrepostas. |
...
| 400 Bad Request | Order.Calculation.NotPending | The order in the state has no calculations to be done. | A reserva no state não tem cálculos a serem feitos. |
| 400 Bad Request | Order.Journeys.Count | The maximum number of Journeys allowed in a order is {0}. | O número máximo de trechos permitidos em uma reserva é {0}. |
...
| 400 Bad Request | Order.State.Timeout | The state time for this order has expired. Please retrieve the order again with the GET Order method. | O tempo em state para esta reserva expirou.  |
| 400 Bad Request | OrderDivide.BalanceDue.Positive | The order in state has a positive balance due. | A reserva no state tem saldo devedor positivo. |
| 400 Bad Request | OrderDivide.CreditAmount.invalid | The informed 'creditAmount' is invalid because it's greater than the amount available in the informed order. | O 'creditAmount' informado é inválido por ser maior que o valor disponível na reserva informada. |
...
| 400 Bad Request | OrderDivide.PassengerKeys.InvalidAmount | The number of 'passengerKeys' in the request must be less than the number of passengers in the informed order. | A quantidade de 'passengerKeys' na solicitação deverá ser menor que a quantidade de passageiros da reserva informada. |
| 400 Bad Request | OrderDivide.Payments.Empty | The order in state does not have informed payments. | A reserva no estado não possui pagamentos informados. |
...
| 400 Bad Request | RequestFailed.Journeys.NotAllowed | The organization fee override is only allowed while adding new journeys to the order. | A substituição da taxa de organização só é permitida ao adicionar novas viagens a reserva. |
...
| 400 Bad Request | RequiredField.Gender | The 'Gender' is required. | O 'Gênero' é obrigatório. |
...
| 400 Bad Request | RequiredField.LastName | The 'LastName' is only required for 'CriteriaType': 'PassengerName' or 'ContactName'. | O 'Sobrenome' só é necessário para 'CriteriaType': 'PassengerName' ou 'ContactName'. |
...
| 400 Bad Request | RequiredField.PassengerKeys | The 'passengerKeys' informed in the request does not exist in the informed order. | As 'passengerKeys' informadas na solicitação não existem na reserva informada. |
...
| 400 Bad Request | Warning.OrganizationFeeOverride | Organization fee override is not applicable for this order. | A substituição da taxa de organização não se aplica a esta reserva. |
...
| 401 Unauthorized | RequestFailed.Authentication | An error occurred while authenticating. | Ocorreu um erro durante a autenticação. |
...
| 404 Not Found | Assistances.NoMatches | Assistance not found in the specified journey. | Assistência não encontrada na viagem especificada. |
...
| 404 Not Found | Code.NoMatches | The informed service is invalid. | O serviço informado é inválido. |
...
|	404 Not Found	| JourneyKey.NotFound	|	 The 'JourneyKey' not found. 	| A 'JourneyKey' não foi encontrada. |
...
| 404 Not Found | Order.NoMatches | The requested order was not found or does not exist. | A reserva solicitadA não foi encontrado ou não existe. |
| 404 Not Found | Order.RetrieveRequired | No order found in state. | Nenhuma order no estate. |
| 404 Not Found | OrderCache.NoMatches | An order must be attached. | Uma reserva deve ser anexado. |
| 404 Not Found | OrderSearch.NoMatches | No orders found for the informed search filters. | Não foram encontradas reservas para os filtros de busca informados. |
| 404 Not Found | Organization.NoMatches | The informed organization is invalid or the logged user does not have permission to use it! | A organização informada é inválida ou o usuário logado não tem permissão para utilizá-la! |
...
| 404 Not Found | PassengerKey.NoMatches | The informed 'PassengerKey' does not match within those on the order in state. | A 'PassengerKey' informada não corresponde àquelas da reserva em state. |
| 404 Not Found | PassengerFeeKey.NotFound | The 'PassengerFeeKey' not found. | O 'PassengerFeeKey' não foi encontrado. |
| 404 Not Found | PassengerName.NoMatches | The Passenger does not match TudoAzul customer number. | O Passageiro não corresponde ao número de cliente TudoAzul. |
| 404 Not Found | Payments.NoMatches | No payments were found for the order in state. | Nenhum pagamento foi encontrado para a reserva no state. |
| 404 Not Found | RecordLocator.NoMatches | Your order must be created to use its credits. | Sua reserva deve ser criado para utilizar seus créditos. |
| 404 Not Found | RecordLocator.NotFound | The 'RecordLocator' not found. | O 'RecordLocator' não foi encontrado. |
...
| 404 Not Found | Service.NoMatches | Service not found in the specified Journey. | Serviço não encontrado no trecho especificada. |
...
| 406 Not Found | Journey.DepartureDate.Passed | It is not possible to remove a journey whose departure date has already passed. | Não é possível remover uma journey cuja data de partida já tenha passado. |
| 409 Conflict | AssignSeat.PassengerKey.Conflict | The PassengerKey does not exists in the order state. | O PassengerKey não existe no state da reserva. |
| 409 Conflict | AssignSeat.Seat.Conflict | Seat Already Assigned. | Assento já atribuído. |
| 409 Conflict | InvalidField.AssistanceCode | The assistance '{0}' is invalid for the Journey '{1}-{2} - {3}'. | A assistência '{0}' é inválida para o trecho '{1}-{2} - {3}'. |
...
| 409 Conflict | LiableRecordLocator.UnaccopaniedMinor.NotMatchJourneys | Liable order and unaccompanied minors order flights must match. | Reservas de responsáveis ​​e voos de reservas de menores desacompanhados devem corresponder. |
| 409 Conflict | NotEmpty.State | It's not possible to attach with an order already retrieved in the current state. Please execute an order delete before the order attach. | Não é possível anexar uma reserva já recuperado no estado atual.  |
| 409 Conflict | Order.HasRecordLocator | This method is for orders under creation process. If you want to checkout a previously created order, please use the equivalent method in the Order.Management.Api. | Este método é para reservas em processo de criação.  |
| 409 Conflict | Order.Recordlocator.Empty | This method is for previously created orders. If you want to retrieve an order under creation, please use the equivalent method in the Order.Api. | Este método é para reservas criados anteriormente.  |
...
| 409 Conflict | Order.UnderCreationException | This method is for orders under creation process. If you want to retrieve a previously created order, please use the equivalent method in the Order.Management.Api. | Este método é para reservas em processo de criação.  |
| 409 Conflict | Refund.NotApplicable | There is no amount in the order to be refunded. | Não há valor na reserva a ser reembolsada. |
...
|	422 Unprocessable Content	|	Checkout.V2.Blocked 	|	 This method is not compatible with retrieve order in V2. 	| Este método não é compatível com order retrieve em V2 |
| 422 Unprocessable Content | NotMatch.Name | The names sent do not match those on the attached order. | Os nomes enviados não correspondem aos da reserva em anexo. |
| 422 Unprocessable Content | NotMatch.PassengerKey | An informed passenger key does not exists in the attached order or is invalid. | A chave do passageiro informada não existe na reserva anexo ou é inválida. |
| 422 Unprocessable Content | Order.RetrieveRequired | The requested order was not found or does not exist. | A reserva solicitada não foi encontrado ou não existe. |
| 422 Unprocessable Content | Order.State.Timeout | The state time for this order has expired. Please retrieve the order again with the GET Order method. | O tempo estadual para esta reserva expirou.  |
|	422 Unprocessable Content	|	 RefundType.CreditsShell.CannotBeApplied 	|	 It is not possible to make the refund using credit shell. 	| Não é possível realizar o reembolso usando credit shell. |
|	422 Unprocessable Content	|	 RefundType.Payments.CannotBeApplied 	|	 There are no amounts to be refunded. 	| Não existem valores para serem reembolsados. |
...
| 502 Bad Gateway | AuthorizationFailed.Credentials | An error occurred while authenticating. | Ocorreu um erro durante a autenticação. |
| 502 Bad Gateway | B2b.OrderManagementRequest.Failed | The request failed. | A requisição falhou. |
| 502 Bad Gateway | Integration.Error | The informed 'RecordLocator' is invalid! | O 'RecordLocator' informado é inválido! |
...
| 502 Bad Gateway | InvalidField.CountryCode | An error occurred while requesting order. | Ocorreu um erro ao solicitar a reserva. |
...
| 502 Bad Gateway | InvalidField.FareKey | The 'FareKey' is invalid. | O 'FareKey' é inválido. |
...
| 502 Bad Gateway | InvalidField.OrganizationCode | The 'OrganizationCode' is invalid. | O 'OrganizationCode' é inválido. |
...
| 502 Bad Gateway | InvalidField.PassengerType.Length | The 'Passengers.Type' has a maximum length of 3 characters. | O 'Passengers.Type' tem comprimento máximo de 3 caracteres. |
...
| 502 Bad Gateway | Navitaire.GetBooking.Failed | The requested order was not found or does not exist. | A reserva solicitada não foi encontrado ou não existe. |
| 502 Bad Gateway | Navitaire.Request.Failed | The request failed. | A solicitação falhou. |
...
| 502 Bad Gateway | Order.RequestFailed | An error has ocurred retrieving the order from state. | Ocorreu um erro ao recuperar a reserva do state. |
...
| 502 Bad Gateway | RequestFailed.Authentication | An error occurred while authenticating. | Ocorreu um erro durante a autenticação. |
...
| 502 Bad Gateway | RequestFailed.CompletedPayment | The amounts for this order have already been totally paid. | Os valores desta reserva já foram integralmente pagos. |
| 502 Bad Gateway | RequestFailed.ContactError | Your Order request was succefully processed, but the informed booking contact has errors. | A sua reserva de encomenda foi processado com sucesso, mas o contacto de reserva informado apresenta erros. |
| 502 Bad Gateway | RequestFailed.DeletingError | An error occurred when deleting the order. | Ocorreu um erro ao excluir a reserva. |
...
| 502 Bad Gateway | RequestFailed.IncompletePayment | The Payment must include at least the total amount of the order taxes. | O Pagamento deve incluir pelo menos o valor total dos impostos da reserva. |
...
| 502 Bad Gateway | RequestFailed.Order.AlreadyExistsInState | There is already an order created in state. | Já existe uma reserva criado no estado. |
| 502 Bad Gateway | RequestFailed.OrderPayments | The order in the current state has not been checked out yet. The CreditType 'Order' only applies to created orders. | A reserva no estado atual ainda não foi verificado.  |
| 502 Bad Gateway | RequestFailed.OverPayment | The informed amount is greater than the balance due for this order. | O valor informado é superior ao saldo devido desta reserva. |
| 502 Bad Gateway | RequestFailed.Payments | An error occurred with the order payments service. | Ocorreu um erro com o serviço de pagamentos de reservas. |
| 502 Bad Gateway | RequestFailed.SSR.AddNotPossible | It was not possible to add an SSR to the order. | Não foi possível adicionar um SSR ao reserva. |
| 502 Bad Gateway | RequestFailed.UnaccompaniedMinor.Information | There are unaccompanied minors in your order, please verify possible costs and required documentations. | Há menores desacompanhados em sua reserva, verifique possíveis custos e documentações exigidas. |
...
| 502 Bad Gateway | UserOrganization.NotAllowed | The logged user organization does not have access to the requested order. | A organização do usuário logado não tem acesso a reserva solicitada. |
